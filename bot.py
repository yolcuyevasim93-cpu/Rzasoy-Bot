...
def dil_secimi_kb():
    ...
    diller = [("🇦🇿 Az", "az"), ("🇹🇷 Tr", "tr"), ("🇷🇺 Ru", "ru"), ("🇺🇿 Uz", "uz"), ("🇬🇧 En", "en"), ("🇩🇪 De", "de")]
    for text, code in diller: b.add(types.InlineKeyboardButton(text=text, callback_data=f"lang_{code}"))
    b.adjust(3)
    return b.as_markup()

def get_game_kb(status="aktiv"):
    b = InlineKeyboardBuilder()
    if status == "gozlemede": b.add(types.InlineKeyboardButton(text="Aparıcı olmaq istəyirəm ✅", callback_data="aparici_ol"))
    else:
...
async def show_rank(c: types.CallbackQuery):
    ...
    if cid not in reytinq_db or not reytinq_db[cid]: return await c.answer("Məlumat yoxdur!", show_alert=True)
    users = sorted(reytinq_db[cid].items(), key=lambda x: x[1][mode], reverse=True)
    txt = f"📊 <b>Reytinq ({'Ümumi' if mode == 'total' else 'Cari Oyun'}):</b>\n\n"
    for i, (uid, d) in enumerate(users[:20]): txt += f"{i+1}. <a href='tg://user?id={uid}'>{d['name']}</a>: {d[mode]} qələbə\n"
...
async def chk(m: types.Message):
    ...
    if cid in aktiv_oyunlar:
        ...
        if msg == "/kick" + BOT_NAME and uid == g["uid"]:
            aktiv_oyunlar[cid]["st"] = "gozlemede"
            await m.answer("Növbə dayandırıldı!", reply_markup=get_game_kb("gozlemede"))
...
async def cb(c: types.CallbackQuery):
    ...
    elif data == "aparici_ol": await yeni_raund(cid, uid, uname, c.message.message_id)
